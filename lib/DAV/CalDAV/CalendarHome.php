...
class CalendarHome  extends \Sabre\CalDAV\CalendarHome
{
     ...
     * @return Calendar
     */
    public function _getChild($name)
    {
        // Special nodes
        if ('inbox' === $name && $this->caldavBackend instanceof \Sabre\CalDAV\Backend\SchedulingSupport) {
            return new \Sabre\CalDAV\Schedule\Inbox($this->caldavBackend, $this->principalInfo['uri']);
        }
        if ('outbox' === $name && $this->caldavBackend instanceof \Sabre\CalDAV\Backend\SchedulingSupport) {
            return new \Sabre\CalDAV\Schedule\Outbox($this->principalInfo['uri']);
        }
        if ('notifications' === $name && $this->caldavBackend instanceof \Sabre\CalDAV\Backend\NotificationSupport) {
        ...
        foreach ($this->caldavBackend->getCalendarsForUser($this->principalInfo['uri']) as $calendar) {
            if ($calendar['uri'] === $name) {
                if ($this->caldavBackend instanceof \Sabre\CalDAV\Backend\SharingSupport) {
                    return new \Afterlogic\DAV\CalDAV\Shared\Calendar($this->caldavBackend, $calendar);
                } else {
