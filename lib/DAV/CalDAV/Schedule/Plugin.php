...
class Plugin extends \Sabre\CalDAV\Schedule\Plugin {
    ...
    public function scheduleLocalDelivery(ITip\Message $iTipMessage)
    {
        ...
        list(, $sToUserPublicId) = \Sabre\Uri\split($principalUri);
        Server::setUser($sToUserPublicId);
        $server = Server::createInstance();

        $home = $server->tree->getNodeForPath($homePath);
        $inbox = $server->tree->getNodeForPath($inboxPath);
        ...
        if ($result) {
            ...
            $objectNode = $server->tree->getNodeForPath($objectPath);
